...
RF_tuned = RandomForestClassifier(min_impurity_split=1, n_estimators=100, bootstrap= True, max_features=15, criterion='entropy', min_samples_leaf=10, max_depth=None).fit(train_data, train_labels)
RF_calibrated_and_tuned_pre_fit = CalibratedClassifierCV(RF_tuned, method = 'isotonic', cv = 'prefit')
RF_calibrated_and_tuned = RF_calibrated_and_tuned_pre_fit.fit(calibration_data,Calibration_labels)

param_grid = {"base_estimator": [RF_calibrated_and_tuned],
